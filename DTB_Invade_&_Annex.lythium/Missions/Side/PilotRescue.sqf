...
["We received a distress call from a friendly heli. We believe the enemy has shot it down, your job is to find and rescue the pilot who is believed to have survived the crash. Be careful when approaching the search area we expect heavy AA."
,"Side Mission: search and rescue","sideCircle"],
(getMarkerPos "sideCircle"),"Created",0,true,"heal",true] call BIS_fnc_taskCreate;
...
while { sideMissionUp } do {
	...
	if ((_bleedouttimer == 0)||(!alive RescueMe)) exitWith {
		...
		RescueMe setPosWorld [28263.7,3703.05,41.5028];
		RescueMe setDir 3.295;
        sleep 5;
