player addEventHandler ["HandleDamage", {
    params ["_unit", "_selection", "_damage", "_source", "_weapon", "_index","_instigatorobj"];
    ...
    if (alive _unit) then {
        ...
        if (!isNull objectParent _unit && {!alive vehicle _unit}) exitWith {
			...
			if (isPlayer _source) then {
				...
				_instigator = name _instigatorobj;
				[_killed,_killer,_instigator,_weapon,_source,_instigatorobj,_source] remoteExec ["AW_fnc_killMessage",0,false];
			};
            forceRespawn _unit;
        };
